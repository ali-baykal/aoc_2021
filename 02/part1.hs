...
goDown x Position{depth = d, horizontal = h} = Position {depth = d + x, horizontal= h}

goUp :: Int -> Position -> Position
goUp x Position{depth = d, horizontal = h} = Position {depth = d - x, horizontal= h}

goForward :: Int -> Position -> Position
goForward x Position{depth = d, horizontal = h} = Position {depth = d, horizontal= h + x}
...
main = do 
    ...
    let finalPosition = foldl (\acc fn -> fn acc) Position{horizontal = 0, depth =0} modifiers
