...
simulate population days = 
    simulate nextGen (days - 1)
    where
        decreased = map (\timer -> timer - 1) population
        completedPopulation = filter (>= 0) decreased
        timerUpCount = length population - length completedPopulation
        nextGen = completedPopulation ++ replicate timerUpCount 6 ++ replicate timerUpCount 8